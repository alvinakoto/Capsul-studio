...
- **Page « Travaux »** (a remplacé la projection patrimoniale, plus la dernière page depuis l'ajout de Conclusion) : `projects.travaux_postes` = ids du catalogue `lib/data/travaux.ts`, sélectionnés par chips dans BlocC ; icône + libellé par poste, budget travaux et DPE actuel → visé en KPI. Page omise si aucun poste. Commentaire libre optionnel (`projects.commentaire_travaux`, saisi dans le wizard — Bloc C « Finances », carte « Travaux & ameublement », 400 caractères max) affiché en bas de page si renseigné. Enregistré avec le reste du wizard (`createProject`/`updateProject`), recopié par `duplicateProject`. La projection patrimoniale reste uniquement dans le rapport analytique.
...
- Commentaire travaux perdu après saisie : le champ était dans `ScenarioPanel`, **sous** le bouton « Calculer », mais n'était persisté que par `updateProjectScenario()` au clic sur « Calculer », et seulement si le calcul aboutissait. Le chargé calculait d'abord, écrivait son commentaire ensuite, puis téléchargeait la fiche ou quittait la page : le texte n'était jamais enregistré, sans aucun message. Corrigé en déplaçant le champ dans le wizard (Bloc C, carte « Travaux & ameublement »), enregistré par « Enregistrer » comme toutes les autres données du projet ; `ScenarioPanel` n'écrit plus `commentaire_travaux`. `duplicateProject` ne recopiait pas non plus ce champ, corrigé au passage. **Leçon : un champ qui n'est sauvegardé que par un bouton d'action (« Calculer ») et non par un bouton d'enregistrement se perd dès que l'utilisateur le remplit après l'action — tout champ de contenu doit vivre là où se fait l'enregistrement (wizard), pas dans le simulateur.**
...
- Trigger `on_auth_user_created` sur `auth.users` (fonction `public.handle_new_user()`) : peuple `public.users` à l'inscription — voir section Authentification (S9) ci-dessous
...
- **Création de la ligne `public.users` gérée par trigger, pas côté client** : un trigger Postgres `on_auth_user_created` (fonction `handle_new_user()`, `SECURITY DEFINER`) sur `auth.users` crée automatiquement la ligne `public.users` (`full_name` lu depuis `raw_user_meta_data`) à chaque inscription. Nécessaire car la RLS de `public.users` (`id = auth.uid()`) bloquait silencieusement l'ancien `upsert()` fait côté client dans `app/inscription/page.tsx` : depuis que "Confirm email" est actif, `signUp()` ne renvoie aucune session tant que le lien n'est pas cliqué, donc `auth.uid()` valait `null` au moment de l'upsert — d'où des chargés dont le nom complet saisi à l'inscription n'apparaissait jamais dans la sidebar (repli silencieux sur `email.split('@')[0]`). Les comptes créés avant l'activation de "Confirm email" n'étaient pas affectés (session immédiate à l'époque).
- **Site URL / Redirect URLs Supabase** (Authentication → URL Configuration, dashboard uniquement) : si l'URL de prod (`https://studio.capsul-france.com`) n'est pas dans la liste "Redirect URLs", Supabase ignore silencieusement le `emailRedirectTo` passé par le code et retombe sur le "Site URL" du projet — resté par défaut sur `http://localhost:3000` sur un nouveau projet. Symptôme observé : le chargé clique le lien de confirmation reçu par email et atterrit sur `localhost` au lieu du site en prod. Penser à whitelister aussi les URLs de preview Vercel (`https://capsul-studio-*.vercel.app/**`) si les liens doivent aussi fonctionner depuis une preview.
